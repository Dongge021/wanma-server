...
app.get('/api/export', async (req, res) => {
  ...
  all.forEach(x => (x.data||[]).forEach(d => {
    csv += `"${x.distributor}","${x.dsrName}","${d.name}","${d.area}","${d.addr}","${d.tel}","${s[d.status]||''}","${d.sysName||''}","${d.sCode||''}"\n`;
  }));
...
app.get('/admin', async (req, res) => {
  const all = await readData();
  const s = {1:'已合作+已建档',2:'已合作+名不同',3:'已合作+未建档',4:'未合作+已建档',5:'未合作+未建档',6:'该店不存在'};
  let h = '<meta charset=UTF-8><title>万马奔腾后台</title><meta name=viewport content="width=device-width,initial-scale=1">' +
  ...
  all.forEach(x => {
    ...
    h += `<tr><td>${x.dsrName}</td><td>${x.distributor}</td><td>${new Date(x.timestamp).toLocaleString()}</td><td>${d}/${x.data.length}</td></tr>`;
  });
  res.send(h + '</table>');
});
