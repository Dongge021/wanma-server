...
app.get('/api/export', async (req, res) => {
  ...
  all.forEach(x => (x.data||[]).forEach(d => {
    const isNew = d.isNew ? '是' : '';
    csv += f`"${x.distributor}","${x.dsrName}","${d.name}","${d.area}","${d.addr}","${d.tel}","${sm[d.status]||''}","${d.sysName||''}","${d.sCode||''}","${d.isNew?'新增':''} \n`;
  }));
...
app.get('/admin', async (req, res) => {
  const all = await readData();
  const sm = {1:'已合作+已建档',2:'已合作+名不同',3:'已合作+未建档',4:'未合作+已建档',5:'未合作+未建档',6:'该店不存在'};
  let html = '<meta charset=UTF-8><title>万马奔腾后台</title><meta name=viewport content="width=device-width,initial-scale=1"><style>body{font-family:-apple-system,Microsoft YaHei,sans-serif;background:#f5f6fa;margin:20px}table{border-collapse:collapse;width:100%;background:#fff;border-radius:8px}th{background:#1a1a2e;color:#fff;padding:10px 12px;font-size:13px;text-align:left}td{padding:8px 12px;font-size:12px;border-bottom:1px solid #eee}.new-row{background:#fff8e1!important}td.new{background:#fff3cd;color:#856404;font-weight:bold}.btn{padding:8px 20px;background:#e94560;color:#fff;border:none;border-radius:6px;font-size:14px;text-decoration:none;display:inline-block;margin:10px 0}</style><h1>🐎 万马奔腾后台</h1><a class=btn href=/api/export>📥 导出全部CSV</a>';
  let totals = {};
  all.forEach(x => totals[x.distributor] = (totals[x.distributor]||0)+(x.data||[]).length);
  html += '<div style="display:flex;gap:20px;margin:16px 0">';
  Object.entries(totals).forEach(([k,v]) => { html += `<div style="background:#fff;padding:16px 20px;border-radius:8px;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="font-size:24px;font-weight:700">${v}</div><div style="font-size:12px;color:#888">${k}</div></div>`; });
  html += `<div style="background:#fff;padding:16px 20px;border-radius:8px;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="font-size:24px;font-weight:700">${all.length}</div><div style="font-size:12px;color:#888">提交人数</div></div></div>`;
  html += '<table><tr><th>DSR</th><th>经销商</th><th>时间</th><th>确认数</th></tr>';
  all.forEach(x => {
    const d = (x.data||[]).filter(d => d.status).length;
    html += `<tr><td>${x.dsrName}</td><td>${x.distributor}</td><td>${new Date(x.timestamp).toLocaleString()}</td><td>${d}/${x.data.length}</td></tr>`;
  });
  res.send(html + '</table>');
});
