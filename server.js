...
const TK_A = 'gho_AuW2LFxTj3njEKhl';
const TK_B = 'f05iF3CvjPxMfn21Uv8F';
...
app.get('/api/export', async (req, res) => {
  const all = await readData();
  const st = {1:'已合作+已建档',2:'已合作+名不同',3:'已合作+未建档',4:'未合作+已建档',5:'未合作+未建档',6:'该店不存在'};
  let csv = '\uFEFF经销商,DSR,门店名称,区域,地址,电话,状态,系统内名称,S编码,来源\n';
  all.forEach(x => (x.data||[]).forEach(d => {
    csv += `"${x.distributor}","${x.dsrName}","${d.name}","${d.area}","${d.addr}","${d.tel}","${st[d.status]||''}","${d.sysName||''}","${d.sCode||''}","${d.isNew?'网页搜索新增':'原始名单'}"\n`;
  }));
...
app.get('/admin', async (req, res) => {
  ...
  let h = '<meta charset=UTF-8><title>万马奔腾后台</title><meta name=viewport content="width=device-width,initial-scale=1"><style>' +
    'body{font-family:-apple-system,Microsoft YaHei,sans-serif;background:#f5f6fa;margin:20px}' +
    'table{border-collapse:collapse;width:100%;background:#fff;border-radius:8px;margin-bottom:24px}' +
    ...
    '.new-row{background:#fff8e1}' +
    '.tag{display:inline-block;font-size:10px;background:#ffc107;color:#333;padding:1px 5px;border-radius:3px;margin-left:3px}' +
    ...
    '</style><h1>🐎 万马奔腾后台</h1><a class=btn href=/api/export>📥 导出全部CSV</a>';
  let total = {};
  all.forEach(x => total[x.distributor] = (total[x.distributor]||0)+(x.data||[]).length);
  h += '<div style="display:flex;gap:20px;margin:16px 0">';
  Object.entries(total).forEach(([k,v]) => { h += `<div style="background:#fff;padding:16px 20px;border-radius:8px;box-shadow:0 1px 3px rgba(0,0,0,.1)"><div style="font-size:24px;font-weight:700">${v}</div><div style="font-size:12px;color:#888">${k}</div></div>`; });
  ...
  all.forEach(x => {
    ...
    const newCount = (x.data||[]).filter(d => d.isNew).length;
    h += `<h2 style="font-size:15px;color:#1a1a2e;margin-top:20px">${x.dsrName} - ${x.distributor} <span style="font-size:12px;color:#888">${done}/${x.data.length} (新增${newCount})</span></h2>`;
    h += '<table><tr><th>门店名称</th><th>区域</th><th>状态</th><th>系统内名称</th><th>S编码</th><th>来源</th></tr>';
    (x.data||[]).forEach(d => {
      const cls = d.isNew ? ' class="new-row"' : '';
      const tag = d.isNew ? '<span class="tag">新增</span>' : '';
      h += `<tr${cls}><td>${d.name}${tag}</td><td>${d.area||''}</td><td>${st[d.status]||''}</td><td>${d.sysName||''}</td><td>${d.sCode||''}</td><td>${d.isNew?'网页搜索新增':'原始名单'}</td></tr>`;
    });
    h += '</table>';
  });
  res.send(h);
